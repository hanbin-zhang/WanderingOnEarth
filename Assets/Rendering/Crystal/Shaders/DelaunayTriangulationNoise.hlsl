...
// Use "Parabolic lifting" method to calculate if two triangles are about to flip. This is actually more reliable than circumscribed circle method. The technique is based on duality between Delaunay Triangulation and Convex Hull, where DT is just a boundary of convex hull of projected seeds onto paraboloid. We project (h1 h2 h3) triangle ontot paraboloid and return the distance of the origin to a plane crossing projected triangle.
