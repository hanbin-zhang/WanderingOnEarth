...
#pragma domain domain
	...
	{
		Varyings output;
		float Noise = tex2Dlod(_Noise, float4(input.uv + _Time.x, 0, 0)).r;
    float NoiseSmaller = tex2Dlod(_Noise, float4(input.uv * 2 + _Time.x, 0, 0)).r;
    float combineNoise = (Noise + NoiseSmaller) * 0.5;
		input.vertex.xyz += (input.normal) *  combineNoise * _Weight;
...
#define DomainPos(fieldName) v.fieldName = \
    ...
    float NoiseSmaller = tex2Dlod(_Noise, float4(IN.uv * 2 + _Time.x, 0, 0)).r;
    float combineNoise = (Noise + NoiseSmaller) * 0.5;
